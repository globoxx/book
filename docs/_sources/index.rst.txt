.. image:: ../static/assets/modulo-head-banner.png
   :alt: "Bannière"

Informatique au gymnase : Travaux pratiques
===========================================

Bienvenue sur la plateforme de TP d'informatique !

.. toctree::
   :name: Programmation (parcours formel)
   :caption: Programmation (parcours formel)
   :maxdepth: 3
   :hidden:

   prog_formel/intro
   prog_formel/variables
   prog_formel/fonctions
   prog_formel/definitions
   prog_formel/conditions
   prog_formel/sequences
   prog_formel/algo

.. toctree::
   :name: Programmation (parcours dessin)
   :caption: Programmation (parcours dessin)
   :maxdepth: 3
   :hidden:

   prog_dessin/dessin
   prog_dessin/definir
   prog_dessin/repeter
   prog_dessin/parametrer
   prog_dessin/parcourir
   prog_dessin/randomiser

.. toctree::
   :name: Web
   :caption: Web
   :maxdepth: 3
   :hidden:

   web/html1
   web/html2
   web/html3
   web/html4
   web/css1

.. toctree::
   :name: Autres activités
   :caption: Autres activités
   :maxdepth: 3
   :hidden:

   divers/aventure
   divers/epidemie
   divers/portes_logiques

.. toctree::
   :name: Outils
   :caption: Outils
   :maxdepth: 3
   :hidden:

   outils/code_libre
   outils/glossaire

`Programmation (parcours formel) <prog_formel/intro.html>`_
: ce chapitre contient le parcours d'apprentissage du langage python avec une approche formelle.

`Programmation (parcours dessin) <prog_dessin/intro.html>`_
: ce chapitre contient le parcours d'apprentissage du langage python avec une approche par le dessin (basé sur le contenu de [Modulo](https://dev-apprendre.modulo-info.ch/prog1/index.html)).

`Web <web/html1.html>`_
: ce chapitre contient le parcours d'apprentissage du HTML et CSS pour concevoir des sites web (basé sur l'excellent [tutoriel](https://openclassrooms.com/fr/courses/1603881-creez-votre-site-web-avec-html5-et-css3) de Mathieu Nebra).